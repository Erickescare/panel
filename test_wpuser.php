	...
	<div class="container">
		<div class="content">
			...
			if(isset($_GET['aksi']) == 'delete'){
				...
				$cek = mysqli_query($con, "SELECT * FROM wp_users WHERE ID='$nik'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
				}else{
					$delete = mysqli_query($con, "DELETE FROM wp_users WHERE ID='$nik'");
					if($delete){
			...
			<form class="form-inline" method="get">
				<div class="form-group">
					<select name="filter" class="form-control" onchange="form.submit()">
						<option value="0">Filtros de datos de wp_users</option>
			...
			<table class="table table-striped table-hover">
				...
				<?php
				if($filter){
					$sql = mysqli_query($con, "SELECT * FROM wp_users WHERE estado='$filter' ORDER BY ID ASC");
				}else{
				...
				if(mysqli_num_rows($sql) == 0){
					...
					while($row = mysqli_fetch_assoc($sql)){
						...
						<tr>
							<td>'.$no.'</td>
							<td>'.$row['ID'].'</td>
							<td><a href="profile.php?nik='.$row['ID'].'"><span class="glyphicon glyphicon-user" aria-hidden="true"></span> '.$row['user_nicename'].'</a></td>
